...
Unreleased
==========
- Move primary module from ``quantityfield`` to ``django_pint`` to match the package name; ``quantityfield`` is now a deprecated compatibility shim that re-exports all symbols from ``django_pint`` and emits ``DeprecationWarning`` on import (`issue #9 <https://github.com/CarliJoy/django-pint/issues/9>`_, `#130 <https://github.com/CarliJoy/django-pint/pull/130>`_)
